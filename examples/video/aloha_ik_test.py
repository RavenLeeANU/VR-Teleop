"""Local MuJoCo viewer for testing ALOHA IK targets.

This mirrors ``aloha_video_host.py``: same scene, same gripper sites,
same mink tasks, same velocity/configuration limits, and the same arm
actuator write path.  Instead of live wrist telemetry, this script applies
fixed Cartesian, yaw, or point targets to the gripper site.
"""
...
from aloha_video_host import _ARM_JOINT_NAMES, _DEFAULT_MODEL
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize ALOHA IK responses.")
    parser.add_argument(
        "--model",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Comma-separated direction list, e.g. '+x,+y,-z' or '+yaw,-yaw'.",
    )
    parser.add_argument(
        "--mode",
        choices=("translate", "yaw", "point"),
        default="translate",
        help=(
            "translate offsets the gripper; yaw rotates the target around the arm base; "
            "point tracks a given XYZ target."
        ),
    )
    parser.add_argument(
        "--point",
    ...
    parser.add_argument(
        ...
        type=int,
        default=90,
        help="Simulation steps to hold each target.",
    )
    parser.add_argument(
        "--return-steps",
        type=int,
        default=45,
        help="Simulation steps used to return to neutral between targets.",
    )
    parser.add_argument(
        "--ik-iters",
        type=int,
    ...
    parser.add_argument(
        "--paused",
        action="store_true",
        help="Open viewer at neutral pose and do not run the target sequence.",
    )
    parser.add_argument(
        "--kinematic",
        action="store_true",
...
def _parse_directions(value: str) -> list[str]:
    ...
    invalid = [
        item for item in directions if item not in _DIRECTION_VECTORS and item not in _YAW_DIRECTIONS
    ]
    if invalid:
        valid = ", ".join([*_DIRECTION_VECTORS, *_YAW_DIRECTIONS])
...
def main() -> int:
    ...
    labels = ["point"] if args.mode == "point" else _parse_directions(direction_arg)
    ...
    print(
        f"testing arm={args.arm}, mode={args.mode}, labels={labels}, "
        f"offset={args.offset:.3f}m, yaw_angle={args.yaw_angle:.3f}rad"
    )
    for prefix in active_prefixes:
        home = arms[prefix].home_pos
        print(f"{prefix} home=({home[0]:+.4f}, {home[1]:+.4f}, {home[2]:+.4f})")
        ...
        while viewer.is_running():
            for label in labels:
                ...
                if args.mode == "point":
                    ...
                    if label not in _YAW_DIRECTIONS:
                        print(f"skip {label}: yaw mode only accepts +yaw/-yaw")
                        continue
                    yaw = args.yaw_angle if label == "+yaw" else -args.yaw_angle
                    targets = {
                        prefix: _yaw_target(arms[prefix], yaw) for prefix in active_prefixes
                    }
                else:
                    if label not in _DIRECTION_VECTORS:
                        print(f"skip {label}: translate mode only accepts Cartesian directions")
                        continue
                    vector = _DIRECTION_VECTORS[label] * args.offset
                    targets = {
                ...
                for step in range(args.return_steps):
                    ...
                    if args.mode == "yaw":
                        yaw = (args.yaw_angle if label == "+yaw" else -args.yaw_angle) * alpha
                        return_targets = {
                            prefix: _yaw_target(arms[prefix], yaw)
                            for prefix in active_prefixes
                        }
                    else:
                        vector = _DIRECTION_VECTORS[label] * args.offset * alpha
                        return_targets = {
                            prefix: (arms[prefix].home_pos + vector, arms[prefix].home_mat)
                            for prefix in active_prefixes
                        }
